...
JdbcTemplate jdbcTemplate = ConnectionUtil.getJdbcTemplate();
	...
	public void update1(ScheduleFoodModel scfood) {

		String sql = "update schedule_food_relation SET SCHEDULE_ID=?,FOOD_ID=? where ID=?";
		Object[] params = { scfood.getSch().getId(),scfood.getFood().getId(),scfood.getId() };
		int rows = jdbcTemplate.update(sql, params);
		System.out.println("No of rows updated: " + rows);

	}

	public void update2(ScheduleFoodModel scfood) {

		String sql = "update schedule_food_relation set SET FOOD_COUNT=? where ID=?";
	...
	public List<ScheduleFoodModel> list() {
		String sql = "select * from schedule_food_relation";
